...
* ``PAID_TIER_0_ROLE_ID`` – role assigned to paying customers (default: 1377733286125637754).
...
class PayingTier0(commands.Cog):
    """Cog providing commands to manage paid Tier 0 members."""
    ...
    # ------------------------------------------------------------------
    def assign_tier(self, guild: discord.Guild, user: discord.Member, tier: str = "0") -> dict:
        """
        ...
        """
        # For now, only tier "0" is supported, but this can be expanded.
        if tier == "0":
            role = guild.get_role(self.paid_tier_0_role_id)
            role_name = role.name if role else "Instap"
        else:
    ...
    @app_commands.command(name="instap", description="Mark a user as a tier 0 customer")
    @app_commands.guilds(_GUILD)
    @app_commands.describe(user="The user to give tier 0 customer status")
    async def instap(self, interaction: discord.Interaction, user: discord.Member) -> None:
        """Assign the tier 0 role to a user and record their membership."""
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    "# Je bent nu lid van The System Instap!\n\n"

                    "The System Instap geeft je de volgende voordelen:\n"
                    "- Toegang tot communicatieve kanalen zoals:\n"
                    f"{indent}- <#1377633695325749268>\n" #gesprekken
                    f"{indent}- <#1378431680888705149>\n" #off-topic
                    "- Deelname aan informatief kanaal:\n"
                    f"{indent}- <#1390406327322280058>\n" #het-systeem
                    "- Je kunt chatten met <@1282862220631478454> om kleine dingen te bespreken.\n\n"

                    "Geniet van The System Instap en ontdek wat onze community te bieden heeft.\n"
    ...
    @app_commands.command(name="endinstap", description="Revoke tier 0 customer status from a user")
    @app_commands.guilds(_GUILD)
    @app_commands.describe(user="The user whose tier 0 status should be removed")
    async def endinstap(self, interaction: discord.Interaction, user: discord.Member) -> None:
        """Remove the tier 0 role from a user and delete their membership record."""
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message(
                "This command can only be used in a guild.",
        ...
        if str(user.id) not in self.bot.active_paid:
            await interaction.response.send_message(
                f"{user.display_name} is not currently marked as a tier 0 customer.",
                ephemeral=True,
            )
            return
        ...
        if role is None:
            await interaction.response.send_message(
                "Tier 0 role not found. Please check the TIER_0_ROLE_ID.",
                ephemeral=True,
            )
            return
        ...
        try:
            await user.remove_roles(role, reason="Revoked tier 0 membership")
            embed = discord.Embed(
                description=(
                    "## Je The System Instap status is beëindigd.\n\n"
                    "- Je hebt geen toegang meer tot de voordelen van The System Instap.\n"
...
async def setup(bot: commands.Bot) -> None:
    """Load the PayingTier0 cog."""
    await bot.add_cog(PayingTier0(bot))
    print("PayingTier0 cog loaded")