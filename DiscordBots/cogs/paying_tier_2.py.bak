...
class PayingTier2(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        paid_tier_2_role_env = os.getenv("PAID_TIER_2_ROLE_ID")
        try:
            self.paid_tier_2_role_id: int = int(paid_tier_2_role_env) if paid_tier_2_role_env else 1429883911839809599
        except ValueError:
            self.paid_tier_2_role_id = 1429883911839809599
    ...
    # ------------------------------------------------------------------
    def assign_tier(self, guild: discord.Guild, user: discord.Member, tier: str = "2") -> dict:
        """
        ...
        """
        # For now, only tier "2" is supported, but this can be expanded.
        if tier == "2":
            role = guild.get_role(self.paid_tier_2_role_id)
            role_name = role.name if role else "Groei"
        else:
    ...
    @app_commands.command(name="groei", description="Mark a user as a tier 2 customer")
    @app_commands.guilds(_GUILD)
    @app_commands.describe(user="The user to give tier 2 customer status")
    async def groei(self, interaction: discord.Interaction, user: discord.Member) -> None:
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    ...
                    "- 3 afspraken met <@1282862220631478454> per week om je voortgang te bespreken en doelen te stellen.\n\n"

                    "Geniet van The System Groei en ontdek wat onze community te bieden heeft.\n"
    ...
    @app_commands.command(name="endgroei", description="Revoke tier 2 customer status from a user")
    @app_commands.guilds(_GUILD)
    @app_commands.describe(user="The user whose tier 2 status should be removed")
    async def endgroei(self, interaction: discord.Interaction, user: discord.Member) -> None:
        ...
        if role is None:
            await interaction.response.send_message(
                "Tier 2 role not found. Please check the TIER_2_ROLE_ID.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    ...
                    "- Je hebt geen toegang meer tot de voordelen van The System Groei.\n"
...
async def setup(bot: commands.Bot) -> None:
    """Load the PayingTier2 cog."""
    await bot.add_cog(PayingTier2(bot))
    print("PayingTier2 cog loaded")